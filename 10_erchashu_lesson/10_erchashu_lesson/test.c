...
// 
// 一般来说 空树 的高度为 0 
// 因为我们从高度为1开始计算，有的书籍可能是以高度为0开始计算， 那么空树的高度为-1
// 所以我们应该正确认识这个问题
// 
...
//递归就是大问题包含小问题
//任何一棵树都被分为根和子树 ，而子树又可以被进行同样的划分 自己为根，下面为子树
//直到被划分为不可再分割的叶子节点，如同递归中不能再往下划分的小问题

//树和非树的概念：

//树中
// 1.子树是不相交的，即 同一层中的每个节点互不相连，或者 说每个节点只有一个父亲，兄弟间不能乱搞
...
//2.若规定根节点的层数为1，则深度为h的二叉树的最大结点数是2^k-1
//3.对任何一棵二叉树，如果度为0，则叶结点个数为n0，度为2的分支节点个数为n2，则有n0=n2+1;
//4.若规定根节点的层数为1，具有N个节点的满二叉树的深度，h=log^(n+1) ps: 以2为底n+1的对数
...
void HeapSort2(int* a, int n)
{
	...
	// 
	// 向下调整法的 时间复杂度为O（N）
	// 推理过程： 结合高度h，用满二叉树为例子（因为满二叉树 n=2^h-1)
	// 使用错位相减法得到规律
	// 得出结论时间复杂度O（N)
	...
	//但是在实际运用中   升序打印---大堆    降序打印---小堆
	//看起来说，以小堆为例，应该是 用来进行升序打印的，因为从上到下依次变大
	//但是如果我们取出堆顶元素后，剩下的节点关系全部打乱，父子变兄弟，兄弟变父子
	//所以需要重新建立堆，再用向上调整法，获取新的最小根节点
	//时间复杂度为O(N^2)
	//
	//所以我们采用大堆的方式
	//我们大堆排序后 ，交换堆顶根节点和尾节点位置，取出最大的放到最后面，然后减减
	//消除它在堆中的位置，然后在进行向下调整，找出第二大的节点放到根节点位置
	//其时间复杂度为O(N*logN) 比我们的 向上调整法 O（N^2)的要效率更高
	for (int i = (n - 1 - 1) / 2; i > 0; --i)
	{
		AdjustDown(a,n,i);
	}
...
//比如专业前十名  世界500强 富豪榜 游戏中前100的活跃玩家
//对于topk问题，能想到的最简单直接的方式就是排序，但是： 如果数据量非常大，排序就不太可去了 
//最佳方式就是用堆来解决
// 最大的前K个  方法：
//1.最优排序  O（N*logN）
//2.建立n个数的大堆， 进行top /pop操作 总共k次   O（N+logN*k)  //建立大堆 N + 向下调整logN*k
//3.假设N非常大，比如N是100亿，K比较小，K是100，如何求解？
//我们通过 建立大堆和小堆调节
//欸嘿
